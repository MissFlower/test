...
function resolvePromise(promise2,x,resolve,reject) {
  ...
  if(x instanceof MyPromise) {
    ...
    // 原因： resolve始终都是用户去调用的，无论同步还是异步 当成功后用户会调用resolve(params) 这样就是触发了resolve函数 params即为value  参数是用户传进来的就是value值
    // value => resolve(value)相当于多写了一个函数 流程相当于:外面用户调用resolve(params) => x.then中的resolve函数接收 这里是触发value => resolve(value) params即为value 然后再去触发resolve(value)
    // => 简化
